...
from utils.cvt2rgb_save import rgb_save, save_error_map, var_rgb_save
...
def train_FSA_T(cfg):
    ...
    # image settings
    # image_n_channel=cfg.image_n_channel
    ...
    comment = "FSA-T"
    logger = TensorboardLogger(place = f'./runs', file_dir=f'./logs', file_logger_name="{}-{}".format(stf_time, comment))
    writer = SummaryWriter(f"./logs/{dataset_name}/{stf_time}")
    ...
    if prior_net_weight:
        ...
        print(f"load pretrain weight from {pretrain_weight_path}")
    ...
    if pretrain_weight is not None:
        ...
        print("load pretrain main weight from {}".format(pretrain_weight))
    ...
    while iterations <= max_iterations:
        for i, dict_data in enumerate(tqdm(dl_train, desc = f"Epoch {epochs}",unit =" batch"), 1):
            pan = dict_data['pan'].cuda(); hr = dict_data['gt'].cuda()
            lms = dict_data['lms'].cuda(); wavelets = dict_data['wavelets_dcp'].cuda()
            with torch.no_grad():
            ...
            # DataParallel 때문에 diff_loss가 나뉘어짐  
            diff_loss = diff_loss.mean()
            diff_loss.backward()

            recon_x = recon_x + lms

            writer.add_scalar('Train/Loss', diff_loss, iterations)
            ...
            if iterations % 2500 == 0:
                ...
                with torch.no_grad():
                    for i, dict_data in enumerate(dl_valid, 1):
                        ...
                        sr, var_map = ema_updater.ema_model(cond, mode="ddim_sample", section_counts="ddim25")
                        ...
                        stack_var = einops.rearrange(var_map, 'b c h w -> c (b h) w')
                        ...
                        #variance map
                        var_rgb_save(stack_var, save_dir=os.path.join(save_dir,'variance'), file_name = f'testimg{i}.png')
                ...
                torch.save(
                    ...
                    f"{ckpt_save_dir}/diffusion_{dataset_name}_iter_{iterations}.pth",
                )
                # ema save
                torch.save(
                    ema_updater.ema_model_state_dict,
                    f"{ckpt_save_dir}/ema_diffusion_{dataset_name}_iter_{iterations}.pth",
                )
                # prenetwork save
