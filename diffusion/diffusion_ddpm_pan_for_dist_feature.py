...
from utils.loss_utils import HybridL1SSIM, HybridL1Frequency, HybridL1SSIMSAM, Beta_nll_Loss
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        assert loss_type in ["l1", "l2", "l1ssim", "l1ssimsam", "l1freq", 'beta_nll']
        # p2 loss weight
    ...
    def set_loss(self, device):
        ...
        elif self.loss_type == "l1freq":
            self.loss_func = HybridL1Frequency().to(device)
        elif self.loss_type == "l1ssimsam":
            self.loss_func = HybridL1SSIMSAM().to(device)
    ...
    def p_mean_variance(
        ...
        *,
        ...
        if condition_x is not None:
            ...
            if self.pred_mode == "noise":
                ...
                if isinstance(model_out, dict):
                    ...
                    x_others = {'variance' : model_out['variance'],
        ...
        return model_mean, posterior_variance, posterior_log_variance, x_recon, x_others
    ...
    def ddim_sample(
        ...
        *_, pred_x_0, pred_others = self.p_mean_variance(x, t, clip_denoised, condition_x, self_cond)
        ...
        # variance ver (08.12)
        pred_variance = pred_others['variance']
        pred_dist_feature_map = pred_others['dist_feature_map']
        return sample, pred_variance, pred_dist_feature_map
    ...
    def ddim_sample_loop(self, x_in, section_counts="ddim300", eta=0.0):
        ...
        if not self.conditional:
            ...
            # for i in reversed(range(0, len(self.betas))):
                ...
                img, variance, dist_feature_map = self.ddim_sample(      # condition ver (08.12)
                    img,
            ...
            # return img    
            return img, variance, dist_feature_map
        else:
            ...
            # for i in reversed(range(0, len(self.betas))):/
                ...
                img, variance, dist_feature_map = self.ddim_sample(      # variance ver (08.12)
                    img,
    ...
    def p_losses(self, x_start, noise=None, cond=None):
        ...
        if self.pred_mode == "noise":
            ...
            if self.loss_type == 'beta_nll':
                ...
                # variance 는 confidence_map의 역수
                # epsilon = 1e-10
                # variance = 1 / (confidence_map + epsilon)
                # variance = 1 - confidence_map
                variance = variance_map
                loss = self.loss_func(x_start, pred, variance, 0.5)
            else:
