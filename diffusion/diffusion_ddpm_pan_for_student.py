...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        # assert loss_type in ["l1", "l2", "l1ssim", "l1ssimsam", "l1freq", 'beta_nll', 'hard_loss', 'None']
        # p2 loss weight
    ...
    def set_loss(self, device):
        if self.loss_type == "l1":
            ...
            self.loss_func = Beta_nll_Loss().to(device)
        elif self.loss_type == 'none':
            self.loss_func = 'no lossfunction'
        else:
        ...
        *,
        ...
        if condition_x is not None:
            ...
            if self.pred_mode == "noise":
                ...
                # variance map size 에 맞게 수정
                if isinstance(model_out, dict):
                    x_recon = model_out['out']
                    # x_variance = model_out['variance']
                    dist_feature_map = model_out['dist_feature_map']
                else:
                    x_recon = model_out
                # x_recon = model_out
            ...
            if self.pred_mode == "noise":
                ...
                # # variance map size 에 맞게 수정
                # if model_out.shape[1] == 4+1 or model_out.shape[1] == 8+1:
                #     x_recon = model_out[:,:-1,...]
                # else:
                #     x_recon = model_out
                if isinstance(model_out, dict):
        ...
        # original ver
        return model_mean, posterior_variance, posterior_log_variance, x_recon, dist_feature_map
        
        # beta nll loss ver (08.12)
        # return model_mean, posterior_variance, posterior_log_variance, x_recon, x_variance
    ...
    def ddim_sample(
        ...
        # condidence map ver
        # *_, pred_x_0, pred_variance = self.p_mean_variance(x, t, clip_denoised, condition_x, self_cond)

        # distillation with feature ver
        *_, pred_x_0, pred_dist_feature_map = self.p_mean_variance(x, t, clip_denoised, condition_x, self_cond)
        ...
        # variance ver (08.12)
        return sample, pred_dist_feature_map
    ...
    def ddim_sample_loop(self, x_in, section_counts="ddim300", eta=0.0):
        ...
        if not self.conditional:
            ...
            for i in tqdm(
                ...
                # img = self.ddim_sample(       # original ver        
                # img, variance = self.ddim_sample(      # variance ver (08.12)     
                img, dist_feature_map = self.ddim_sample(      # condition ver (08.12)
                    img,
            ...
            for i in tqdm(
                ...
                x_device = x['lms'].device if isinstance(x, dict) else x.device

                # img = self.ddim_sample(       # original ver             
                # img, variance = self.ddim_sample(      # variance ver (08.12)
                img, dist_feature_map = self.ddim_sample(
                    img,
                    torch.full((b,), i, device=x_device, dtype=torch.long),
                    condition_x=x,
                    self_cond=self_cond,
                    eta=eta,
                )
    ...
    def p_losses(self, x_start, noise=None, cond=None):
        ...
        if self.self_condition and random.random() < 0.5:
            with torch.no_grad():
                ...
                if self.pred_mode == "noise":
                    ...
                    # variance map 을 위해 수정한 부분
                    if isinstance(model_out, dict):
                        x_self_cond = model_out['out']
                    else:    
        ...
        if self.pred_mode == "noise":
            ...
            if isinstance(model_predict, dict):
                recon_x0 = model_predict['out']
                recon_dist_feature_map = model_predict['dist_feature_map']
            else:
                recon_x0 = model_predict
            
            #loss 가 beta nll 일때 variance map 도 함께 구해야함
            if self.loss_type == 'beta_nll':
                # output N ch + variance map 1 ch or N ch
                pred = model_predict['out']
                variance_map = model_predict['variance']
                variance = variance_map
                loss = self.loss_func(x_start, pred, variance, 0)
            elif self.loss_type == 'hard_loss':
                loss = self.loss_func(x_start, model_predict, variance_map)
            elif self.loss_type == 'none':
                loss = None
            else:
        ...
        return loss, recon_x0, recon_dist_feature_map
